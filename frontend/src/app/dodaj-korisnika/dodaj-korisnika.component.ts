...
export class DodajKorisnikaComponent implements OnInit {
  ...
  register(){
    this.servis.register(this.registerForm.get('username').value, this.registerForm.get('password').value, this.registerForm.get('fullname').value, this.registerForm.get('address').value, this.registerForm.get('phone').value, this.registerForm.get('mail').value,this.photo,parseInt(this.registerForm.get('type').value)).subscribe(ob=>{
        if(ob['message']=='ok'){
          alert("User added!")
          this.ruter.navigate(['/administrator/dodaj-korisnika'])
          .then(() => {
